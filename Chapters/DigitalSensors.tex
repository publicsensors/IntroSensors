...
i2c.scan()    # read the ADC and print the result\end{lstlisting}
		...
		In other situations, including Adafruit's \htmladdnormallink{summary of default \i2c addresses}{https://cdn-learn.adafruit.com/downloads/pdf/i2c-addresses.pdf}, addresses are listed in \textbf{hexadecimal} notation.
...
\labsec{i2c_add_gpios}
\i2c devices can also be used to provide additional digital \gpios.
Because these operate through an \i2c bus, they do not typically operate quickly enough to perform very fast switching tasks, such as the Pulsed Width Modulation (PWM) used to ``snooze'' an LED (\refsec{LED_PWM}).
Nonetheless, they respond quickly enoungh -- typically over a few milliseconds -- to perform many useful functions.
With microcontrollers such as the \esp8266 that have relatively few \gpios, these expanders use just two \gpios, \texttt{SCL} and \texttt{SDA} but add 8 or 16 \gpios (and, with multiple expanders, potentially many more).
...
With \i2c I/O extenders, especially in combination with \i2c differential extenders (\refsec{i2c_extend}), \gpios to count rotations or switch toggles in these devices can be placed 10s of meters away.

The activity in this section anticipates these types of applications by setting up a counter with plus/minus buttons and LED indicators, using an \texttt{MCP23017} \i2c I/O expander.
It demonstrates, among other things, how to set up interactions between a timer-based interrupt (distinct from the button-based interrupt in \refsec{interrupt}) on the microcontroller and the \texttt{MCP23017}'s built-in interrupt functions.

\subsubsection{\howto Remote Input/Output with an \texttt{MCP23017} I/O expander}
\begin{marginfigure}
	\begin{center}
		\htmladdnormallink{\includegraphics[width=\MFW]{Fritzing/feather_MCP23017_bb_crop.png}}{https://github.com/publicsensors/IntroSensors/blob/digital/Fritzing/feather_MCP23017_bb_crop.png}
	...
	\end{center}
\end{marginfigure}
Begin by reviewing the pin geometry of the \texttt{MCP23017}, which is described in the \htmladdnormallink{datasheet}{https://ww1.microchip.com/downloads/en/devicedoc/20001952c.pdf}.
Refer to the first figure in this datasheet as you make connections in the steps below.

The breadboard layout for this circuit is illustrated in \reffig{i2c_IO}.
...
\begin{enumerate}
  ...
  This places the \i2c, \gpios and other pins on the \texttt{MCP23017} close to where their breadboard connections will be.
	...
	\smallskip
	Be sure to place your buttons in the proper orientation, as in \refsec{interrupt}.

	\item \textbf{Set the \i2c address by jumpering the \texttt{A0}, \texttt{A1} and \texttt{A2} pins to \texttt{GND}.}

	In \reffig{i2c_IO}, these are the pins at the upper left of the IC.
  ...
  \item \textbf{Connect two LEDs, with their negative terminals in the \texttt{GND} rail and their positive terminals in \texttt{MCP23017} \gpios 8 and 9.}

	These LEDs will provide visual feedback about the states of the \texttt{MCP23017}'s and \esp8266's interrupts.

  \item \textbf{Use jumpers to connect one terminal of each button to \texttt{GND}, and the other terminals to \texttt{MCP23017} \gpios 4 and 7.}

  \item \textbf{Download the \Micropython driver for the \texttt{MCP23017} by \texttt{@mcauser}, \htmladdnormallink{\texttt{mcp23017.py}}{https://github.com/publicsensors/IntroSensors/blob/main/Codes/mcp23017.py}, and copy it onto your microcontroller.}

	Explanations of this code, along with examples, are at his \htmladdnormallink{github repository}{https://github.com/mcauser/micropython-mcp23017}.

	\item \textbf{Download the \texttt{mcp\textunderscore count.py} script, and copy it onto your microcontroller.}
	\lstinputlisting[language=Python,label=sd_hello,caption={\htmladdnormallink{\texttt{mcp\textunderscore count.py}}{https://github.com/publicsensors/IntroSensors/blob/main/Codes/mcp_count.py}: An example demonstrating use of an \texttt{MCP23017} \i2c I/O expander to increment and decrement a counter, with LED feedback, on button pushes.}]{Codes/mcp_count.py}
...
What are the steps in performing these tasks?
\begin{itemize}
...
\item Lines 10-11 command the \texttt{MCP23017} to set the state of \gpios 4 and 7 to act as inputs, to be pulled up, and to trigger the \texttt{MCP23017}'s internal interrupt when they change state.
These commands use one of three different ways of sending commands to the \texttt{MCP23017}, as explained in \htmladdnormallink{the documentation}{https://github.com/mcauser/micropython-mcp23017}.
...
Unlike the interrupt in \refsec{interrupt}, which was triggered by a button push, this interrupt fires off at regular intervals (once every 50 milliseconds) based on a timer.
...
This is done by reading the value returned by the \lstinline{interrupt_triggered_gpio} method (port 0 refers to \gpios 0-7; port 1 refers to \gpios 8-15).

In lines 10-11, \gpios 4 and 7 (the ones attached to Switches \#1 and \#2 in \reffig{i2c_IO}) are enabled to trigger the interrupt.
...
The \texttt{MCP23017} signals that no interrupt has been triggered by returning a 0.
...
In this case, if \gpio 4 (Switch \#1) triggered the interrupt, the \texttt{MCP23017} returns $2^4$, which is 16.
If \gpio 7 (Switch \#2) triggered the interrupt, it returns $2^7$, which is 128.
...
For port 0, as in this case, this means the first bit represents \gpio 0, the second bit represents \gpio 1, \etc
This value is recorded in the function as \lstinline{intcap0}, and also printed out.

When neither button is pressed, both \gpios 4 and 7 are pulled high.
...
For example, if \gpio 4 is pressed but \gpio 7 is not, \lstinline{intcap0} has the value \lstinline{0b10000000}.
If only \gpio 7 is pressed, \lstinline{intcap0} has the value \lstinline{0b00010000}.

The expression \lstinline{(intcap0 & 2**4)} tests whether the bit corresponding to \gpio 4 is on, by finding bits that are on in \emph{both} \lstinline{intcap0} \emph{and} \lstinline{0b00010000} (which is the byte representing $2^4 = 16$).
Similarly, the expression \lstinline{(intcap0 & 2**7)} tests whether the bit corresponding to \gpio 7 is on, by finding bits that are on in \emph{both} \lstinline{intcap0} \emph{and} \lstinline{0b10000000} (which is the byte representing $2^7 = 128$).

When these values indicate the respective buttons are being pushed, the script turns on the appropriate LED (using \gpio 8 for LED \#1 and \gpio 9 for LED \#2).
This is done using alternative syntax, described in \htmladdnormallink{the documentation}{https://github.com/mcauser/micropython-mcp23017}.
...
\loadMilestone{mlst:05d2} % load milestone with tags id: mlst:04c
