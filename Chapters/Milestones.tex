...
%<*mlst:04b2>
\begin{kaobox}[backgroundcolor=\MScolor,frametitlebackgroundcolor=\MScolor,frametitle=\showmscntr Milestone: Blowing out an LED]
	\begin{itemize}
		\item [$\Box$] This Milestone is to assemble, test and find suitable parameters for a circuit to ``blow out an LED'' as described in this section.
		The key steps are:
		\begin{itemize}
			\item[$\circ$] Assemble and double check the circuit.
			\item[$\circ$] Run the \lstinline{LED_background} function to identify the approximate \adc reading for a lit LED in still air.
			\item[$\circ$] Execute the \lstinline{LED_thresh} method with various choices of the parameters \lstinline{n}, \lstinline{ns} \lstinline{threshold} and \lstinline{timeout_ms}. Experiment to find combinations that respond as quickly and reliably as possible by turning the LED off momentarily when you blow on it.
			\item[$\circ$] When you have a parameter combination that works well, conduct a ``ministudy'' by varying a single parameter at a time. Take note of what happens when you increase or decrease each parameter individually -- if the circuit works better, more poorly or not at all, try to identify the cause (e.g., is the threshold never reached or reached too often, is there excessive noise in the samples, is response time too slow, \etc).
		\end{itemize}
		When you have completed your experiments, complete the assignment by uploading: (a) an image or labeled sketch of your breadboard layout; (b) your choice of ``best'' parameters; and, (c) your diagnosis of the effects of the systematic parameter changes.
	\end{itemize}
\end{kaobox}
%</mlst:04b2>
...
%<*mlst:05d2>
\begin{kaobox}[backgroundcolor=\MScolor,frametitlebackgroundcolor=\MScolor,frametitle=\showmscntr Milestone: Making an \i2c I/O expander-based counter]
	\begin{itemize}
		\item [$\Box$] This Milestone is to implement a \texttt{MCP23017}-based counter as described in this section, and to modify it by adding a button to reset the counter. \,
		\begin{itemize}
			\item[$\circ$] Assemble a circuit similar to \reffig{i2c_IO}, upload the provided codes.
			\item[$\circ$] Test it to demonstrate that the circuit and software works as intended.
			\item[$\circ$] Design and assemble a modified circuit, in which you add a third button and LED. Connect these new elements to the \texttt{MCP23017} in an analogous way (but to different \gpios) as in the original circuit.
			\item[$\circ$] Save a copy of the \lstinline{mcp_count.py} script under a different name. Modify the new script to respond to presses of the new button by illuminating the new LED, and to releases of the new button by extinguishing the new LED and resetting the counter to 0. \,
		\end{itemize}
		When your new circuit and script are working correctly, complete the assignment by uploading: (a) a photo or sketch of your modified counter circuit; (b) your code; and, (c) the output you obtained from your REPL session showing increments, decrements and reset of the counter.
	\end{itemize}
\end{kaobox}
%</mlst:05d2>
